...
public class PrefabControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameManager.Comm.GetDataAction -= Simulation_Control;
        GameManager.Comm.GetDataAction += Simulation_Control;
    }
    ...
    public void Simulation_Control(string ID, int home, int store, string state)
    {
        GameObject SelectObject = GameObject.Find(ID);

        SelectObject.GetComponent<ObjectDefine>().home = home;
        SelectObject.GetComponent<ObjectDefine>().store = store;
        SelectObject.GetComponent<ObjectDefine>().state = state;


        SelectObject.GetComponent<Player>().enabled = true;
        SelectObject.GetComponent<Player>().PrefabUpdate(ID, state);
